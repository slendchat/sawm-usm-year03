...
### Учётная запись менеджера для проверки ролей:
...
2. Роли: гость, пользователь, менеджер, администратор.
...
5. Редактирование, удаление и изменение статуса тикета — только для сотрудников.
   Менеджеры теперь обладают теми же правами управления тикетами (создание, редактирование, удаление, смена статуса), но не могут управлять администраторскими учетками.
6. Раздел `/users` (``User Directory``) доступен администраторам и менеджерам: можно просматривать учетные записи, редактировать email и удалять пользователей (кроме себя и админов для менеджеров).
7. Поиск и фильтрация по заголовку, категории и приоритету.
8. Валидация на клиенте (HTML5, JavaScript) и сервере (PHP).
9. Защита: PDO с подготовленными запросами, `htmlspecialchars`, CSRF-токены.
...
* **Менеджер:** после авторизации получает доступ к полному списку тикетов, может редактировать/удалять записи и менять статусы, а также управляет обычными учетными записями в разделе `/users`.
* **Администратор:** управляет тикетами всех статусов, создаёт новых админов, редактирует и удаляет записи, доступен весь функционал менеджера + панель создания админов.
...
| role          | enum         | YES  |     | NULL              |                   | Роль (`admin`, `manager`)     |
...
5. **Работа менеджера с учетными записями:**

   * Залогиниться `manager/manager`, открыть `/users`, отредактировать email любого обычного пользователя или удалить запись.
